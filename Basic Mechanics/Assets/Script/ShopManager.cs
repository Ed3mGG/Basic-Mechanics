...
public class ShopManager : MonoBehaviour
{
    ...
    {   
        // Suprrime les potentiels boutons présent dans le parent  --> .childCount | .GetChild(i)
        for (int i = 0; i < sellButtonsParent.childCount; i++)
        {
            Destroy(sellButtonsParent.GetChild(i).gameObject);
        }

        // Instancie un bouton pour chaque item à vendre et le configure selon la Prefab
